...
def prepare_data_fake_real(data):
...
def prepare_data_fake_fake(data):
...
class TextDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name in ['birds', 'coco']:
            img_name = '%s/images/%s.jpg' % (data_dir, key)
        else:
            img_name = '%s/jpg/%s.jpg' % (data_dir, key.split("/")[-1])
        ...
        if self.contrastive_type == "fake_fake":
            sent_ix = random.randint(0, self.embeddings_num)
            new_sent_ix = index * self.embeddings_num + sent_ix
...
class TextDataset_Test(data.Dataset):
    def __init__(self, data_dir, split='train',
                 base_size=64,
                 transform=None, target_transform=None, sent_ix=0):
        self.transform = transform
        self.norm = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    ...
    def load_class_id(self, data_dir, total_num):
        if os.path.isfile(data_dir + '/class_info.pickle'):
            with open(data_dir + '/class_info.pickle', 'rb') as f:
                class_id = pickle.load(f, encoding='latin1')  # coding: utf-8
        else:
            class_id = np.arange(total_num)
        return class_id
    ...
    def __getitem__(self, index):
        ...
        imgs = get_imgs(img_name, self.imsize,
                        bbox, self.transform, normalize=self.norm)
        caps, cap_len = self.get_caption(index * self.embeddings_num + self.sent_ix)
        return imgs, caps, cap_len, cls_id, key
